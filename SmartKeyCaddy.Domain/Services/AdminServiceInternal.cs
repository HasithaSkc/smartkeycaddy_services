...
public partial class AdminService
{
    private async Task<DeviceConfigurationMessage> GetDeviceConfigurationMessage(Device device)
    {
        ...
        var keyFobTagsWithRooms = keyFobTags.Where(fob=> fob.PropertyRoomId.HasValue).ToList();
        var deviceSettings = await _deviceRepository.GetDeviceSettings(device.DeviceId);
        ...
        var deviceConfiguratioNMessage = new DeviceConfigurationMessage()
        {
            ...
            Bins = bins,
            KeyFobTags = keyFobTagsWithRooms,
